...
itensParaValores(CelObjeto *iniFilaItens)
{
    ...
    for(p = iniFilaItens->prox; p!= NULL; p = p->prox){
        c = p->valor.pStr;
        if(p->categoria >= 0 && p->categoria <= 16){
            /*Operadores correspondem aos itens 0 a 16 da lista de categorias*/
    ...
    if(iniPosfixa != NULL && iniPosfixa->prox != NULL){
        ...
        while ( p!= NULL ){
            ...
            case OPER_ADICAO:
                ...
                res->categoria = FLOAT;
                res->valor.vFloat = stackPop(s).valor.vFloat + stackPop(s).valor.vFloat;
                stackPush(s, res);
                p = p->prox;
                break;
            case OPER_SUBTRACAO:
                res = mallocSafe(sizeof(CelObjeto));
                res->categoria = FLOAT;
                num = stackPop(s).valor.vFloat;
                res->valor.vFloat = stackPop(s).valor.vFloat - num;
                stackPush(s, res);
                p = p->prox;
                break;
            ...
            case OPER_RESTO_DIVISAO:
                ...
                res->categoria = FLOAT;
                num = stackPop(s).valor.vFloat;
                res->valor.vFloat = (float) fmod(stackPop(s).valor.vFloat, num );
                stackPush(s, res);
                p = p->prox;
                break;
            case OPER_EXPONENCIACAO:
                ...
                res->categoria = FLOAT;
                num = stackPop(s).valor.vFloat;
                res->valor.vFloat = pow( stackPop(s).valor.vFloat, num );
                stackPush(s, res);
                p = p->prox;
                break;
            case OPER_MENOS_UNARIO:
                res = mallocSafe(sizeof(CelObjeto));
                res->categoria = FLOAT;
                res->valor.vFloat = stackPop(s).valor.vFloat * -1;
                stackPush(s, res);
                p = p->prox;
                break;
            case OPER_DIFERENTE:
                res = mallocSafe(sizeof(CelObjeto));
                res->categoria = BOOL;
                if(stackPop(s).valor.vFloat != stackPop(s).valor.vFloat)
                    res->valor.vFloat = 1;
                else
                    res->valor.vFloat = 0;
                stackPush(s, res);
            ...
            case OPER_MAIOR:
                ...
                res->categoria = BOOL;
                num = stackPop(s).valor.vFloat;
                if( stackPop(s).valor.vFloat > num )
                    res->valor.vFloat = 1;
                else
                    res->valor.vFloat = 0;
                stackPush(s, res);
            ...
            case OPER_DIVISAO_INT:
                ...
                res->categoria = FLOAT;
                num = stackPop(s).valor.vFloat;
                res->valor.vFloat = (int) ( stackPop(s).valor.vFloat / num );
                stackPush(s, res);
                p = p->prox;
                break;
