...
{
    if (pObj->valor.pStr != NULL && (pObj->categoria == 24 || pObj->categoria == 25 || pObj->categoria == 30) )
...
freeListaObjetos(CelObjeto *iniLista)
{
    ...
    if(iniLista != NULL && iniLista->prox != NULL) p = iniLista->prox->prox;
    /*começa a liberar a memoria a partir da primeira
    célula útil da lista (após a cabeça)*/
    while(p != NULL){
        q = p->prox;
        freeObjeto(p);
        p = q;
